...
class ProductDetailsView(View):
    def get(self,request,*args,**kwargs):
        ...
        return render(request,'Category/detail.html',{'product':product,'simiar_products':simiar_products,'comments':comments,'buys':buys,'number':number})
# @method_decorator(cache_page(5*60),name='dispatch')
class SearchCategoryView(View):
    ...
    def get(self,request,category_slug,type_slug=None,name_product=None,search=None):
        ...
        'buys':buys,'p':int(p),'lst_product':lst_name,'page_type':page_type,'order_convert_to_farsi':order_convert_to_farsi,'search_name_product':search_name_product,'order':order})
...
class ShopView(View):
    ...
    def get(self,request):
        ...
        if is_ajax ==False:
            ...
            return render(request,'Category/shop.html',{'brands':self.brands,'categories':self.categories,'products':products,'p':int(page_number)})
        if is_ajax:
